  ...
  return (
    <div>
        {habitaciones.map(({id_habitacion, nombre, imagen, capacidad, cantidad_camas}) => (
            ...
            <CardHabitacionesDisponibles nombre={nombre} imagen={validarImagen(imagen)}  capacidad={capacidad} cantidad_camas={cantidad_camas} precio_persona={30000} funcion={()=>seleccionarHab(id_habitacion)}/>
            </div>
        ))}
    </div>
  )
